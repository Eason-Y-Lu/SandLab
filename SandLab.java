...
public class SandLab {
  ...
  public void step() {
    ...
    // add sand and water interaction, sand will always displace water, so if they
    // touch sand and water will trade places, and will always do down and water
    // will go up
    if (grid[row][col] == SAND) {
      if (row + 1 < grid.length
          && (grid[row + 1][col] == EMPTY || grid[row + 1][col] == WATER || grid[row + 1][col] == ACID)) {
        ...
        grid[row + 1][col] = SAND;
      } else if (row + 1 < grid.length && col + 1 < grid[0].length
          && (grid[row + 1][col + 1] == EMPTY || grid[row + 1][col + 1] == WATER || grid[row + 1][col + 1] == ACID)) {
        if (grid[row + 1][col + 1] == WATER) {
          grid[row][col] = WATER;
        } else if (grid[row + 1][col + 1] == ACID) {
          grid[row][col] = ACID;
        } else {
          grid[row][col] = EMPTY;
        }
        grid[row + 1][col + 1] = SAND;
      } else if (row + 1 < grid.length && col - 1 >= 0
          && (grid[row + 1][col - 1] == EMPTY || grid[row + 1][col - 1] == WATER || grid[row + 1][col - 1] == ACID)) {
        if (grid[row + 1][col - 1] == WATER) {
          grid[row][col] = WATER;
        } else if (grid[row + 1][col - 1] == ACID) {
